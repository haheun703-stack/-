...
def _analyze_investor_df(df) -> dict:
    ...
    # 오늘(장중) 제외하고 마지막 유효 행
    # ★B-68(8/13): 구 코드는 `df[df["기관합계"] != 0]`이었다. 의도는 "장중 미확정 행
    #   제외"였으나, **기관이 진짜 0인 날(보합·무거래)까지 행 자체를 삭제**해서
    #   한 줄에서 결함 4개가 나왔다:
    #   ⑴ 연속일수가 0에서 끊기지 않는다 — 카운트를 끊을 행이 사라져 streak가 옛
    #      값에 영구 고정된다(`for val in reversed(...): if val>0 ... else break`가
    #      만날 0이 없다)
    #   ⑵ 외국인 연속일수를 **기관 컬럼으로 거른 프레임**에서 계산한다
    #   ⑶ `_5d`/`_20d`가 5·20거래일 합이 아니라 "기관≠0인 마지막 5·20행" 합이다
    #      — 컬럼명(`_5d_억`)과 정의가 불일치한다
    #   ⑷ `latest`가 오늘이 아니라 "기관이 마지막으로 0이 아니었던 날"이 된다
    #   실물 피해: 공개 `/smart-money`에 138490이 8/5~8/11 **5거래일 완전 동일값**으로
    #   송출됐다("데이터 없음"이 "streak 15일 유지"로 둔갑해 score 100 상위권).
    #   ★판정 기준을 **전 투자자 동시 0**(= 미수집/장중 미확정)으로 바꾼다.
    #     기관만 0인 날은 정상 관측치이므로 남겨야 streak가 제대로 끊긴다.
    _zero_cols = [c for c in ("기관합계", "외국인합계", "개인") if c in df.columns]
    if _zero_cols:
        _all_zero = (df[_zero_cols] == 0).all(axis=1)
        valid = df[~_all_zero]
    else:
        valid = df
    if valid.empty:
