...
def build_pipeline() -> list[Step]:
    ...
    return [
        ...
        Step("23.5", "Brain 대시보드",         s + "build_brain_upload.py",
             depends=["20.9"]),
        Step("23.6", "타입1 수급릴레이",       s + "scan_type1_relay.py",
             depends=["20.9"], optional=True),
        Step("23.65", "타입2 바닥반등",       s + "scan_type2_bottom.py",
             depends=["20.9"], optional=True),
        Step("23.68", "급락반등 스캔",        s + "scan_crash_bounce.py",
             depends=["20.9"], optional=True),
        Step("23.69", "피보나치 스캔",        s + "scan_fibonacci.py",
             depends=["20.9"], optional=True),
