...
REM 9e단계: 섹터 ETF 거래량 모니터링 (선행지표 5 — 거래량 폭발 감지)
echo [%date% %time%] [9e/31] 섹터 ETF 거래량 모니터링 >> logs\schedule.log
python -u -X utf8 scripts\collect_etf_volume.py >> logs\schedule.log 2>&1
if errorlevel 1 echo [%date% %time%] [9e/31] FAILED >> logs\schedule.log
...
REM 11.1단계: SHIELD 포트폴리오 방어 점검 (BRAIN 전에 실행!)
echo [%date% %time%] [11.1/29] SHIELD 방어 점검 >> logs\schedule.log
...
python -u -X utf8 scripts\run_etf_rotation.py --blind-test --no-telegram >> logs\schedule.log 2>&1
...
REM 25단계: 통합 페이퍼 트레이딩 (scan_buy 추천 → 가상 매매 + FLOWX 업로드)
echo [%date% %time%] [25/30] 통합 페이퍼 트레이딩 >> logs\schedule.log
python -u -X utf8 scripts\paper_trading_unified.py >> logs\schedule.log 2>&1
if errorlevel 1 echo [%date% %time%] [25/30] FAILED >> logs\schedule.log

REM 25.5단계: 금요일 → 주간 리포트
for /f "tokens=1" %%a in ('python -c "from datetime import datetime; print(datetime.now().weekday())"') do set DOW=%%a
if "%DOW%"=="4" (
    echo [%date% %time%] [25.5/30] 페이퍼 주간 리포트 >> logs\schedule.log
