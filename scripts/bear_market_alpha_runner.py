...
def format_message(today: str, regime: dict, alphas: list, inverse: dict, flow_top: list) -> str:
    ...
    if inverse.get("signal"):
        lines.append(f"🔻 *인버스 매수 시그널!* 외인 5일 매도 *{inverse['foreign_5d_eok']:,.0f}억* (임계 {inverse['threshold_eok']:,}억)")
        lines.append(f"• 252670 KODEX 200선물인버스2X (5/12~15 실측 +7.3%)")
        lines.append(f"• 114800 KODEX 인버스 (+3.7%)")
        lines.append("")
