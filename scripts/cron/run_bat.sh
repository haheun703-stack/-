...
case "$BAT" in
  ...
  HEALTH) # 19:20 KST(8/14 B-85로 18:45→19:20) — 자동 복구: BAT-D 완료(18:56~19:10) 후 신선도 확인 → 낡은 파일만 재실행
    ...
    # ★9/19 신설: 「고쳤다」고 보고한 것이 오늘도 고쳐진 상태인가를 매일 다시 본다.
    #   계기 = 9/7에 「B-47 완결」이라 보고한 배선이 **12거래일 동안 한 셀도 채우지
    #   않았다**는 것이 9/19에야 드러난 일. 같은 형태가 9/7에도 3건(B-98·B-97·B-99)
    #   있었고, 최근 신규 부채의 절반 이상이 «내가 완료라고 보고한 것의 실제 상태»다.
    #   조치 당일 라이브 1회 검증은 하지만 **그 뒤로 아무도 다시 보지 않는 것**이 원인.
    #   ★위치: `health_check.py` **앞**이어야 한다 — 뒤에 두면 stale 자동복구가 끝난
    #     상태를 보게 되어 "BAT-D에서 깨져 있었다"는 사실이 사라진다(B-94와 같은 이유).
    #   ★`run_py`를 쓰지 않는다 — 회귀는 «실행 실패»가 아니라 «상태»다. FAIL_COUNT에
    #     섞으면 BAT-HEALTH 실패 알림과 중복되고, 매일 같은 ❌가 되어 B-106처럼
    #     배경이 된다. 자체 텔레그램이 **연속 거래일수**를 붙여 따로 보낸다.
    timeout 600 $PY scripts/verify_resolved_claims.py >> "$LOG" 2>&1 || true
