...
- 교훈 재확인: 라이브 106건 분해가 아무리 그럴듯해도(-2.16% vs +0.57%)
  비이벤트 기저선 없이 규칙화하면 안 된다.

★7/10 전체검수 정정: 위 "2026 손실=레짐 베타" 단정은 강등한다. 로컬 데이터가
2026-06-18에 끝나 라이브 가설의 출처인 6/22~7월 크래시 구간이 백테스트에 없음
(베타 귀속은 크래시 이전 구간에서만 입증됨). 또한 가격축 전수 근사는 페이퍼의
top-5 선별·수급 게이트 계층을 재현하지 못하므로 기각의 사정거리는 가격축 모집단에
한정된다. 기각 자체(2019~25 CAUTION 페널티 부재·66건만으로 규칙화 금지)는 유지.
