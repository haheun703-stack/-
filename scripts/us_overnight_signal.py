...
US_KR_SECTOR_MAP = {
    ...
    "icln": ["태양광", "풍력"],
    # 로봇/자동화 (BOTZ) — 현대로보틱스, 레인보우로보틱스, 두산로보틱스
    "botz": ["로봇자동화"],
    # 방산/항공 (ITA) — 한화에어로스페이스, LIG넥스원, 현대로템
    "ita": ["방산"],
    # IT/SW
    "xlk": ["IT", "소프트웨어"],
    # 금융
...
def _run_level2_pattern(df: pd.DataFrame, latest, prev) -> dict:
    ...
    try:
        from src.utils.us_kr_history import PatternMatcher
    except ImportError:
...
def generate_signal(df: pd.DataFrame | None = None) -> dict:
    ...
    # ─── 10. ETF 전파 모델 — 폐지 (2026-05-17) ───
    # scripts.etf_transmission은 archive/deprecated. CLAUDE.md LOCK 규칙 위반 시도.
    # 매일 WARNING 노이즈 발생 → 옵션 C로 호출 자체 제거. 결과 활용 부분(L1561~)도 함께 제거.
...
def format_telegram_message(signal: dict) -> str:
    ...
    # ETF 전파 모델 출력 — 폐지 (2026-05-17): 호출부 제거에 따른 출력부도 함께 제거.
