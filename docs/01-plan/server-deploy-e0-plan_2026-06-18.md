...
작성: 2026-06-18 (목) · 상태: **사장님 GO 대기** · 연계: [unfreeze-checklist.md](unfreeze-checklist.md) §2-E

> **결정 1줄**: freeze→unfreeze 경로의 첫 마일스톤. 서버 HEAD `12ca898`(6/10) → 현재 `b90ae07` 전체 pull(60커밋)
...
| P2 | **E-0 배선 코딩 + 로컬 테스트** (페이퍼 엔진 → gate_check) | 나 (낮) | ⏳ 착수 대기 |
...
## 3. 배포 실행 절차 (저녁 — 16:10 자동화 확인 + 마감 후)

> ⚠️ **장중 금지.** `quantum-scheduler` 재시작이 장 시간 서버작업을 끊음. 16:10 자동화 정상 확인 후.

```
# 0) 배포 전 서버 스냅샷 (롤백 대비)
ssh ... "cd ~/quantum-master && git rev-parse HEAD && git status --short"
ssh ... "ls -la data/risk/ kill_switch/ 2>/dev/null; ls gate_log_*.jsonl 2>/dev/null | head"

# 1) E-0 커밋 푸시 (로컬) — 낮에 작성·테스트한 E-0 배선
git add -A && git commit && git push   # E-0 포함

# 2) 서버 pull (60커밋: 리스크엔진 Phase2 VaR·gate_wiring·밸류밴드·2층·collect_kis·sector_fire fix·E-0)
ssh ... "cd ~/quantum-master && git pull"

# 3) 의존성 점검 (리스크엔진 신규 import: 서버 venv에 누락 없나)
ssh ... "cd ~/quantum-master && source venv/bin/activate && python -c 'import risk.var_engine, risk.gate_wiring' 2>&1"

# 4) 서비스 재시작
ssh ... "sudo systemctl restart quantum-scheduler"

# 5) 로그 확인
ssh ... "sudo journalctl -u quantum-scheduler --no-pager -n 50"
```
...
- 배포 후 서비스 크래시/import 실패 시: `ssh ... "cd ~/quantum-master && git reset --hard 12ca898 && sudo systemctl restart quantum-scheduler"`.
...
| 시각 | 작업 | 담당 |
|------|------|------|
| 낮 | E-0 배선 코딩 + 로컬 테스트 (P2) | 나 |
| 병행 | KRX_PW 갱신 (P1) | 사장님 |
| 16:10 | 자동화 첫 가동 확인 + 6/18 데이터 6단계 검증 | 자동→나 확인 |
| 마감 후 | 배포 실행(§3) + 검증 3종(§4) | 나 |
