...
- [ ] **E. (스펙 §8 공통)** 페이퍼 트레이딩 최소 20거래일 + 게이트/킬스위치 로그가 빠짐없이 남는지 확인 ⬅ **미착수(아래 E-0 선행 필요)**
  - [ ] **E-0. ★페이퍼 엔진 게이트 드라이런 배선(선행 필수)** — 실제 도는 페이퍼 엔진(`scripts/paper_trading_unified.py`)이 리스크엔진 게이트를 **미경유**(`gate_check` 호출 0). gate_check 배선 6곳은 전부 실매매 경로(smart_entry·adaptive_*·limit_up·live_trading·telegram)뿐. 페이퍼 엔진은 어댑터/잔고포트조차 안 쓰는 자체 체결 시뮬이라, gate_check가 요구하는 `balance_port.fetch_balance()`(equity)·`ohlcv_loader`(adv20)를 페이퍼 잔고로 어댑팅 + enforce 모드 설계가 필요(반나절급). **이 배선이 깔려야 GATE-DRYRUN 로그가 쌓이기 시작** → 그 후 20거래일 누적.
    - 6/16 정보봇 교차검증 확정: 서버 `gate_log_*.jsonl` 0건·`data/risk/` 없음·`kill_switch/state.json` 없음 + 서버 HEAD `12ca898`(6/10)라 게이트 코드 자체 미배포.
    - **[6/17 결정 — 보류, 서버 배포와 묶음]** 사장님 + 정보봇 검증으로 E-0 **보류** 확정. ★**E-0 코드를 로컬 master에 짜도 서버 배포 전엔 효력 0**(서버 페이퍼 엔진이 옛 코드로 돌아 GATE-DRYRUN 로그 0 → 카운트 0). 따라서 **E-0 착수 트리거 = 서버 배포 결정**. 배포가 미뤄진 동안 E-0를 미리 짜는 실익 없음(죽은 코드 선행 = "코드 존재 ≠ 런타임 연결" 함정 재발 계열, line 102 정정과 동일 맥락).
    - **[6/18 — E-0 코드 작성 완료(로컬)·효력은 배포 후]** 사장님 "오늘 저녁 배포" 결정에 따라 E-0 배선 구현: `src/use_cases/paper_gate.py`(`PaperBalancePort` 페이퍼 잔고 어댑터 + `run_paper_gate_dryrun` graceful) + `paper_trading_unified.enter_new_positions` 가상매수 직전 1줄 배선(enforce=False·흐름 무변경·실매매 6호출처 무접촉) + `tests/test_paper_gate_dryrun.py` 10케이스. 실데이터 검증=005930 매수→verdict PASS(adv20 7.4조·VaR95 0.6%)·`gate_logs_paper/gate_log_*.jsonl` 1줄 기록 실증. 회귀 신규실패 0(4건 baseline 동일=날짜만료/sell-path). ★**아직 `[ ]`인 이유**: 서버 배포 전이라 GATE-DRYRUN 로그가 서버에 안 쌓임→20거래일 카운트 미시작. 배포(저녁) 후 서버 `gate_logs_paper` 적재 확인 시 카운트 개시. 계획서=[server-deploy-e0-plan_2026-06-18.md](server-deploy-e0-plan_2026-06-18.md).
    - **[6/22 — 사무실 재추적: 배선 정상 확증, 6/19 카운트는 타이밍 누락]** ★KRX 사태(6/19~22)로 §4 검증이 묻혀 있던 것을 추적. **배선 코드는 정상**(서버 격리 재현 `run_paper_gate_dryrun`→verdict=PASS·`gate_log` 1182B 생성, graceful 실패 아님). **6/19 gate_log 0의 원인=배포 타이밍**: 6/19 페이퍼 cron 18:38:52 실행 > E-0 배포(`5cfb45b`) 18:58:37 — 페이퍼가 20분 먼저 돌아 옛 코드(`12ca898`)로 매수 처리(reflog 대조 확정). 6/22 08:11 `de03137`(⊃ E-0) 재배포 + 서버 venv `import risk/paper_gate/gate_wiring` 전부 OK. **→ 오늘(6/22) 16:30 D cron 페이퍼부터 실카운트 경로 `data/risk/gate_logs_paper`에 적재 시작 = 20거래일 카운트 D-day 후보**(오늘 16:30 후 첫 줄 확인 시 §4-(1) 완료·카운트 공식 개시). ⚠️매수 0인 날은 line 899(enter_new_positions 가상매수 직전) 미도달 → 그날 미카운트(시장상황 의존·정상).
    - ⚠️ **배포할 때 반드시 동시 수행**: (1) E-0 배선 + (2) 서버 배포(⚠️현 서버 6/10 → 전체 pull = 리스크엔진 대량배포라 별도 신중 결정) + (3) 배포 직후 서버 `gate_log_*.jsonl` 적재 시작 확인. 셋 중 하나라도 빠지면 "E가 왜 0?"을 또 재발견(= 6/17 오전 반복).

**현재 상태**: A·B·C(1b-i 자물쇠)·C-ii-a·**C-ii-b(2차 호출처 6개 + 커버리지 테스트)**·D ✅ / **E ⬅ 미완(유일)**. 리스크 엔진 코드 측 잠금·발급·**실매매 경로** 배선·우회커버리지 전부 완료. ⚠️**[6/16 정정] E는 "시간 누적"이 아니다** — 이전 본 문서가 "추가 코드 없이 관측만 쌓으면 됨"이라 적었으나, 실제 페이퍼 엔진이 게이트 배선 밖이라 **게이트 로그가 쌓이는 구조 자체가 없음**(코드 존재 ≠ 런타임 연결). **E는 시간이 아니라 배선(E-0)이 채운다**: E-0(페이퍼 게이트 드라이런 배선) 선행 → 서버 배포 → 그 뒤 20거래일 누적 시작. ⚠️실제 unfreeze 전 별도 확인: ①chart_hero 재배선 시 게이트 추가(D 재점검과 동시) ②시장가 사이즈 바인딩 완화(6/13 적대검증, 실탄 시장가 켜기 전, P2 안전측·fail-closed). ★**B(E를 1회 검증으로 완화)는 §0 "협상 불가·건너뛸 수 없다"(사장님 6/11 확정) 위반이라 배제** — 20거래일은 게이트가 여러 시장상황서 오작동·과차단 없이 도는지 보는 안전 마진.
