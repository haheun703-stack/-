...
## ★D002 백테스트 판정 (2026-07-09 실행 — 12개월·이벤트 6,748건·기저선 16,937건)

**판정: BEAR/CRISIS 조건부 승격 후보** (같은 날 짝지음 초과수익, 날짜클러스터 t):

| 국면 | +5일 | +10일 | +20일 |
|---|---|---|---|
| 2025 하반기(강세) | +0.05%p (t 0.2) | -0.16%p (t -0.4) | -0.38%p (t -0.7) |
| 2026 상반기(폭락) | +0.73%p (t 2.8) | +1.96%p (t 2.4) | **+3.75%p (t 4.9)** |

- 임원 매수는 **공포 국면에서만 정보** — 폭락장에서 임원매수 종목은 무작위 대비
  확실히 덜 빠짐. 강세장에선 무익(무해). 7/7 수급 재정의(연기금=BULL 확인지표)와 거울상.
- 정직 고지: 중앙값 차 ~0 (평균 엣지는 소수 종목 큰 승리가 견인·극단값 제거 후 방향 유지),
  12개월 = 국면 2개뿐 (다년 확장 검증 여지 남음).
- 승격 시: 레짐 BEAR/CRISIS 한정 + 보조 가중치(10~20%)로 시작. forward 관찰 병행 (cron 19:10).
