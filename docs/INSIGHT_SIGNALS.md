...
## ★D002 백테스트 판정 (2026-07-10 전체검수로 정정 — 승격 보류)

**판정: 승격 보류 (7/9의 "BEAR/CRISIS 조건부 승격 후보" 판정은 7/10 적대검수로 기각됨).**
forward 관찰(cron 19:10)은 계속 — 관찰은 무해하며 판단 근거를 누적한다.

7/9 판정이 무너진 4가지 이유 (검수 재계산으로 전부 CONFIRMED):
1. **"2026H1=폭락장" 라벨이 허위**: 해당 반기 KOSPI +96.7% (사상급 랠리). 진짜 폭락(6/22~)은
   +20d 완결 표본에 사실상 없음(마지막 완결 공시일 6/10). "공포 국면에서 임원매수가 방어"
   서사는 측정 구간과 무관한 최근 3주 폭락에 앵커링된 사후 스토리텔링이었음.
2. **레짐 조건부 근거 없음**: 이벤트일을 실제 index_regime으로 매핑하면 2026H1의 63%가
   CAUTION, CRISIS는 2일뿐. 엣지는 BULL/CAUTION/BEAR에 균일(+3.5~4.1%p) — "BEAR/CRISIS
   한정 가점" 규칙은 관측된 엣지 대부분(CAUTION 버킷)을 버리는 규칙이 됨.
3. **종목 클러스터 미반영**: 기저선이 실제 206종목 고정 재사용(설계 400, 무음 드롭)이라
   날짜클러스터 t=4.9는 과대. 종목단위 Welch + 윈저5% 후 t=1.11(ns). +20d 엣지 합의 68%가
   8종목(우측꼬리 복권) 기여. 중앙값 이벤트 종목은 기저선보다 오히려 약간 나쁨.
4. **자체 사전 기준 미달**: 아래 판정 가이드("초과 중앙값 모두 양수 + 승률차 +5%p") 재계산
   결과 중앙값 3구간 전부 음수·승률차 +4.5~4.6%p 미달. 기준 탈락 후 사전에 없던 반기 분리를
   도입해 채택한 것 자체가 절차 위반.

정직한 재기술: "2026H1 특이 장세(지수 2배 랠리)에서 임원매수 종목의 평균 초과 +2~4%p는
소수 급등주의 우측꼬리가 견인했고, 중앙값 0·국면 조건부 증거 없음·사전 기준 미달 —
승격 근거 없음. forward 관찰 + 향후 다년/타국면 재검증으로만 재론."
★교훈: 국면 라벨은 반드시 측정 창의 지수 수익률로 검증할 것(캘린더 반기 ≠ 레짐).
채택 판정은 반드시 사전 기준 통과 후에만.
...
- KIS 토큰은 `data/insight_signals/kis_token.json`에 캐시. (7/9 검수 결론:
  기존 봇은 파일 캐시 없이 메모리 캐시만 사용 → 공유할 파일이 없어 자체 캐시가 정답.
  실행 시각 19:10은 기존 봇 발급 시간대와 겹치지 않음. 상단 '공유 필수' 문단은
  파일 캐시가 존재할 경우의 지침.)
