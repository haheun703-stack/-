...
# ═══════════════════════════════════════════════════════
# STEP 9: MARKET JOURNAL — 장중 기록 + 학습 + 주간/월간 보고
# ═══════════════════════════════════════════════════════
# 목적: 매일 장 마감 후 시장 기록 자동화, 주간/월간 보고서 자동 발송
#       SQLite 누적 → 패턴 학습 → 파라미터 개선 제안 (3개월 후)
# 원칙: BAT-D 마지막 단계에 추가, 별도 BAT 불필요
#       금요일 → 주간보고, 월말 → 월간보고 (market_journal.py 내부 분기)

- [x] 9-1. 일간 기록 엔진
  - 파일: scripts/market_journal.py
  - 시장 개요(KOSPI/KOSDAQ/레짐/환율/VIX/수급)
  - 급등/급락/거래량폭발/수급쏠림 TOP5 자동 추출
  - 보유 종목 일지 + EYE 감지 연동
  - 매매 실적 (있으면)
  - SQLite(data/market_journal/journal.db) + JSON 이중 저장
  - 완료일: 2026-03-20

- [x] 9-2. 일간 학습 엔진
  - 패턴 매칭: 오늘 vs 과거 유사 조건 → 다음날 통계
  - 급등주 사후 추적: 3일/5일 후 수익률 누적
  - 봇 시그널 정확도: 등급별 적중률 업데이트
  - 수급 신호 유효성: 쌍끌이/외인단독/기관단독 통계
  - 저장: data/market_journal/insights/*.json + SQLite
  - 완료일: 2026-03-20

- [x] 9-3. 일간 리포트 (텔레그램)
  - send_evening_summary.py에 _section_journal() 추가
  - 시장 요약 + 급등/급락 TOP3 + 보유 현황 + EYE + 학습 1줄
  - 완료일: 2026-03-20

- [x] 9-4. 주간 보고서
  - market_journal.py 내부 generate_weekly_report()
  - 금요일 자동 실행 (BAT-D 23.9단계)
  - 주간 성과/MVP/WORST/EYE요약/학습인사이트
  - 텔레그램 분할 발송 (4096자 제한)
  - 완료일: 2026-03-20

- [x] 9-5. 월간 보고서
  - market_journal.py 내부 generate_monthly_report()
  - 월말 영업일 자동 실행
  - 월간 성과/레짐변화/시그널적중률/수급통계/개선제안
  - 완료일: 2026-03-20

- [x] 9-6. BAT-D 연동
  - BAT-D 23.9단계: python scripts/market_journal.py
  - 24단계(저녁통합) 직전 실행
  - 금요일/월말 자동 판별 (별도 BAT 불필요)
  - 완료일: 2026-03-20
