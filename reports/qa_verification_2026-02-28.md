...
|------|------|-------------|------|-----|------|
| 2. 트레이딩 로직 | 11 | 4 | 0 | 0 | 15 |
| 3. 슬리피지/체결 | 4 | 0 | 1 | 4 | 9 |
| 4. 데이터 무결성 | 12 | 0 | 1 | 0 | 13 |
| 5. 시스템 안정성 | 4 | 3 | 5 | 0 | 12 |
| **합계** | **31** | **7** | **7** | **4** | **49** |

**판정: 재검수 필요 (NO-GO)** — Critical 항목 없으나, Fail 7건 존재
...
| 2 | 최대 동시 포지션 | **Conditional** | **KOSPI 레짐 슬롯(0~5) 계산은 정확하나 LiveTradingEngine MAX_POSITIONS=5 하드코딩. 레짐 캡 미연결** |
...
| 5 | 텔레그램 장애 | **Fail** | **대체 알림 경로 전무. 긴급 상황 알림 블라인드 리스크** |
...
1. **KOSPI 레짐 캡 미연결** (2.3-2)
   - LiveTradingEngine.max_positions=5 하드코딩
   - CRISIS(0슬롯)에서도 매수 가능한 구조적 결함
   - 권고: execute_buy_signals()에서 KOSPI 레짐 슬롯 조회 후 available_slots 적용

2. **텔레그램 대체 알림 없음** (5.2-5)
   - 긴급 청산 시 텔레그램 장애면 사용자 알림 불가
   - 권고: 파일 기반 알림 로그 + Windows 알림 또는 이메일 대체 경로
...
- **실전 투입: 재검수 필요 (NO-GO)**
- **사유**: Critical 2건(레짐 캡 미연결, 텔레그램 대체), 중요 3건(MDD 연동, 백업, 피드 감지)
- **예상 수정 소요**: Critical 2건 수정 후 재검수 → GO 가능
