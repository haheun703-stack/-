...
"""
    ...
    def _format_cfo() -> str:
        ...
        # 원장 라벨 (7/16): cfo_report.json은 메인A(paper_portfolio.json) 기준이라
        # 본 브레인의 계좌(예: 블루칩 10종목)와 종목 수가 다른 게 정상 —
        # 라벨 없이는 AI가 "데이터 정합성 문제"로 오인해 매일 경고를 생성함.
        lines = ["[CFO 포트폴리오 건강 리포트 — 메인A 계좌 기준 (본 계좌와 다른 원장이므로 종목 수 불일치는 정상)]"]
