"""국적별 외국인 수급 시그널 엔진 v3 — 7 Secrets + Retail Flow Edition

KRX 국적별 거래량 + KIS 투자자별 순매수 + 주가 데이터 → 고정밀 매수/매도 시그널
...
+ Secret #8: 개인 수급 역류 (Retail Counter-Flow)
  — 개인 지지 / 개인 추격 / 개인 항복 / 스마트머니 괴리 / 개인 주도 감지
  — 외국인이 빠져도 개인이 받쳐주는 종목 vs 개인이 추격하는 함정 분별
...
# KIS 투자자별 매매동향 (개인 수급)
try:
    from src.adapters.kis_investor_adapter import fetch_investor_batch
...
class NationalitySignal:
    """종목별 국적 수급 시그널 (v3 — 7 Secrets + Retail Flow)."""
    ticker: str
    name: str
    ...
    # Secret #8: 개인 수급 역류 (v3)
    retail_net_5d: int = 0           # 개인 5일 순매수 (원)
    retail_net_20d: int = 0          # 개인 20일 순매수 (원)
    retail_foreign_ratio: float = 0.0  # 흡수율: 개인매수/외국인매도 (0~∞, 1.0=완전 흡수)
    retail_consecutive: int = 0      # 연속 순매수(+)/순매도(-) 일수
    retail_pattern: str = ""         # RETAIL_SUPPORT / FOMO / CAPITULATION / SMART_DIVERGE / DRIVEN
...
def _load_retail_data(tickers: list[str]) -> dict[str, pd.DataFrame]:
    """KIS API에서 투자자별 매매동향(개인/외국인/기관) 배치 로드.

    Returns:
        {ticker: DataFrame(date, 외국인합계, 기관합계, 개인, ...)}
    """
    if not KIS_INVESTOR_AVAILABLE:
        logger.warning("KIS 투자자 어댑터 미사용 — 개인 수급 분석 스킵")
        return {}

    try:
        return fetch_investor_batch(tickers, delay=0.08)
    except Exception as e:
        logger.warning(f"KIS 투자자 배치 로드 실패: {e}")
        return {}


def _calc_retail_metrics(inv_df: pd.DataFrame | None) -> dict:
    """KIS 투자자 데이터에서 개인 수급 핵심 지표 산출.

    Returns:
        {
            "retail_net_5d": int,          # 개인 5일 순매수 (원)
            "retail_net_20d": int,         # 개인 20일 순매수 (원)
            "foreign_net_5d_amt": int,     # 외국인 5일 순매수 (원)
            "inst_net_5d_amt": int,        # 기관 5일 순매수 (원)
            "retail_consecutive": int,     # 연속 순매수(+)/순매도(-) 일수
            "retail_acceleration": float,  # 개인 거래량 가속도
            "retail_foreign_ratio": float, # 외국인 매도 흡수율 (0~∞)
        }
    """
    empty = {
        "retail_net_5d": 0, "retail_net_20d": 0,
        "foreign_net_5d_amt": 0, "inst_net_5d_amt": 0,
        "retail_consecutive": 0, "retail_acceleration": 0.0,
        "retail_foreign_ratio": 0.0,
    }
    if inv_df is None or inv_df.empty or "개인" not in inv_df.columns:
        return empty
    ...
    try:
        ...
        # 연속 순매수/순매도
        retail_series = inv_df["개인"].values
        consecutive = 0
        for val in reversed(retail_series):
            if val > 0:
                if consecutive >= 0:
                    consecutive += 1
                else:
        ...
        # 가속도 (최근 5일 개인 순매수 시계열)
        retail_vals = inv_df["개인"].tail(7).tolist()
        acceleration = 0.0
        if len(retail_vals) >= 4:
            deltas = [retail_vals[i] - retail_vals[i - 1] for i in range(1, len(retail_vals))]
            accels = [deltas[i] - deltas[i - 1] for i in range(1, len(deltas))]
            if accels:
                recent_accel = sum(accels[-2:]) / len(accels[-2:])
                avg_abs = sum(abs(v) for v in retail_vals) / len(retail_vals)
                acceleration = recent_accel / avg_abs if avg_abs > 0 else 0.0

        # 흡수율: 외국인 매도 중 개인이 얼마나 받아주는가
        # 외국인 매도(음수) × 개인 매수(양수) → ratio > 0 = 개인이 흡수 중
        absorption = 0.0
        if foreign_5d < 0 and retail_5d > 0:
            absorption = retail_5d / abs(foreign_5d)
        elif foreign_5d > 0 and retail_5d < 0:
            # 역방향: 외국인 매수 + 개인 매도 → 음수 흡수율 (스마트머니 진입)
            absorption = -(abs(retail_5d) / foreign_5d)

        return {
            "retail_net_5d": retail_5d,
            "retail_net_20d": retail_20d,
            "foreign_net_5d_amt": foreign_5d,
            "inst_net_5d_amt": inst_5d,
            "retail_consecutive": consecutive,
            "retail_acceleration": round(acceleration, 4),
            "retail_foreign_ratio": round(absorption, 3),
        }
    except Exception as e:
        logger.warning(f"개인 수급 지표 산출 실패: {e}")
        return empty


def _detect_retail_pattern(
    retail_metrics: dict,
    price_change_pct: float,
    inst_trend: float,
) -> tuple[str, int, str]:
    ...
    Patterns:
        RETAIL_SUPPORT — 외국인 매도 + 개인 매수 > 70% 흡수 (개인 지지)
        RETAIL_FOMO    — 주가↑ + 개인 급매수 + 기관/외국인 이탈 (추격 매수, 위험)
        RETAIL_CAPITUL — 장기 개인 매수 후 급매도 전환 (항복 = 잠재 바닥)
        SMART_DIVERGE  — 기관↑ + 개인↓ (스마트머니 괴리)
        RETAIL_DRIVEN  — 개인이 절대적 주도 (불안정)
        NONE           — 패턴 없음

    Returns:
        (pattern, score_delta, reason)
    """
    r = retail_metrics
    ret_5d = r["retail_net_5d"]
    fgn_5d = r["foreign_net_5d_amt"]
    inst_5d = r["inst_net_5d_amt"]
    ratio = r["retail_foreign_ratio"]
    consec = r["retail_consecutive"]
    accel = r["retail_acceleration"]

    # 1) SMART_DIVERGE: 기관+외국인 매수 + 개인 매도 → 최고 기회
    if (fgn_5d > 0 or inst_5d > 0) and ret_5d < 0 and inst_trend > 0.05:
        smart_sum = fgn_5d + inst_5d
        if smart_sum > 0 and abs(ret_5d) > smart_sum * 0.3:
            return ("SMART_DIVERGE", 5,
                    f"스마트머니 괴리 (기관+외국인 매수 vs 개인 {ret_5d / 1e8:+,.0f}억 매도)")

    # 2) RETAIL_CAPITULATION: 개인이 오래 사다가 갑자기 매도 전환
    if consec <= -2 and r["retail_net_20d"] > 0 and ret_5d < 0:
        return ("RETAIL_CAPITUL", 4,
                f"개인 항복 (20일 순매수 후 {abs(consec)}일 연속 매도 전환 → 바닥 신호?)")

    # 3) RETAIL_FOMO: 주가 상승 + 개인 가속 매수 + 외국인/기관 이탈
    if price_change_pct > 2.0 and ret_5d > 0 and accel > 0.10:
        if fgn_5d < 0 or inst_5d < 0:
            return ("RETAIL_FOMO", -5,
                    f"개인 추격 매수 (주가+{price_change_pct:.1f}% + 개인 가속 + 스마트머니 이탈)")

    # 4) RETAIL_SUPPORT: 외국인 매도 + 개인 흡수 > 70%
    if fgn_5d < 0 and ret_5d > 0 and ratio >= 0.7:
        pts = 2 if ratio < 1.5 else 3  # 초과 흡수 시 +3
        consec_str = f"{consec}일 연속 매수" if consec > 0 else "매수 중"
        return ("RETAIL_SUPPORT", pts,
                f"개인 지지 (외국인 매도 {ratio * 100:.0f}% 흡수, 개인 {consec_str})")

    # 5) RETAIL_DRIVEN: 개인이 압도적 주도 (외국인+기관 합산의 2배 이상)
    smart_abs = abs(fgn_5d) + abs(inst_5d)
    if smart_abs > 0 and abs(ret_5d) > smart_abs * 2.0 and ret_5d > 0:
        if fgn_5d <= 0 and inst_5d <= 0:
            return ("RETAIL_DRIVEN", -2,
                    f"개인 주도 (스마트머니 부재, 개인 {ret_5d / 1e8:+,.0f}억 독주)")
...
def analyze_stock(
    ...
    retail_df: pd.DataFrame | None = None,
) -> NationalitySignal | None:
    """단일 종목 7 Secrets + Retail Flow 통합 분석."""
    ...
    # --- Secret #8: 개인 수급 역류 (최대 +5 / -5) ---
    retail_metrics = _calc_retail_metrics(retail_df)
    retail_pattern, retail_pts, retail_reason = _detect_retail_pattern(
        retail_metrics, price_change, inst_trend,
    )
    if retail_pts != 0:
        score += retail_pts
        reasons.append(retail_reason)
    ...
    return NationalitySignal(
        ...
        retail_net_5d=retail_metrics.get("retail_net_5d", 0),
        retail_net_20d=retail_metrics.get("retail_net_20d", 0),
        retail_foreign_ratio=retail_metrics.get("retail_foreign_ratio", 0.0),
        retail_consecutive=retail_metrics.get("retail_consecutive", 0),
...
def run_analysis(db_path: Path = DB_PATH) -> list[NationalitySignal]:
    ...
    # Secret #8: 개인 수급 데이터 배치 로드 (KIS API)
    ticker_codes = [t for t, _ in tickers]
    retail_data = _load_retail_data(ticker_codes)
    if retail_data:
        loaded = sum(1 for df in retail_data.values() if not df.empty)
        logger.info(f"개인 수급 데이터 로드: {loaded}/{len(ticker_codes)}종목")
    else:
        logger.info("개인 수급 데이터 없음 — Secret #8 스킵")

    signals = []
    for ticker, name in tickers:
        kis = kis_foreign.get(ticker)
        r_df = retail_data.get(ticker)
        sig = analyze_stock(conn, ticker, name, kis, r_df)
        if sig:
            signals.append(sig)
    ...
    output = {
        ...
        "retail_summary": _summarize_retail(signals),
    }
...
def _summarize_retail(signals: list[NationalitySignal]) -> dict:
    """개인 수급 요약 통계."""
    retail_patterns: dict[str, list[str]] = {}
    total_retail_buy = 0
    total_retail_sell = 0
    for sig in signals:
        if sig.retail_pattern:
            if sig.retail_pattern not in retail_patterns:
                retail_patterns[sig.retail_pattern] = []
            retail_patterns[sig.retail_pattern].append(sig.name)
        if sig.retail_net_5d > 0:
            total_retail_buy += 1
        elif sig.retail_net_5d < 0:
            total_retail_sell += 1

    return {
        "patterns": retail_patterns,
        "retail_buying_stocks": total_retail_buy,
        "retail_selling_stocks": total_retail_sell,
        "support_count": len(retail_patterns.get("RETAIL_SUPPORT", [])),
        "fomo_count": len(retail_patterns.get("RETAIL_FOMO", [])),
        "smart_diverge_count": len(retail_patterns.get("SMART_DIVERGE", [])),
    }
...
def format_telegram(signals: list[NationalitySignal]) -> str:
    ...
    RETAIL_LABEL = {
        "RETAIL_SUPPORT": "👥 개인 지지",
        "RETAIL_FOMO": "🚨 개인 추격",
        "RETAIL_CAPITUL": "💀 개인 항복",
        "SMART_DIVERGE": "🧠 스마트머니 괴리",
        "RETAIL_DRIVEN": "⚠️ 개인 주도",
    }

    lines = ["<b>🌍 국적별 수급 v3 (7 Secrets + 개인 역류)</b>", ""]
    ...
    for sig in active[:15]:  # 상위 15개
        ...
        # 개인 패턴 라벨 추가
        ret_label = RETAIL_LABEL.get(sig.retail_pattern, "")
        ret_str = f" {ret_label}" if ret_label else ""

        lines.append(f"{emoji} <b>{label}</b>  {sig.name}  {score_str}점{pat_str}{ret_str}")
    ...
    # Secret #8: 개인 수급 역류 요약
    retail_support = [s for s in signals if s.retail_pattern == "RETAIL_SUPPORT"]
    retail_fomo = [s for s in signals if s.retail_pattern == "RETAIL_FOMO"]
    retail_capitul = [s for s in signals if s.retail_pattern == "RETAIL_CAPITUL"]
    smart_diverge = [s for s in signals if s.retail_pattern == "SMART_DIVERGE"]
    retail_driven = [s for s in signals if s.retail_pattern == "RETAIL_DRIVEN"]

    has_retail = retail_support or retail_fomo or retail_capitul or smart_diverge or retail_driven
    if has_retail:
        lines.append("<b>👥 개인 수급 역류</b>")
        if smart_diverge:
            names = ", ".join(s.name for s in smart_diverge[:3])
            lines.append(f"  🧠 스마트머니 괴리: {names}")
        if retail_capitul:
            names = ", ".join(s.name for s in retail_capitul[:3])
            lines.append(f"  💀 개인 항복 (바닥?): {names}")
        if retail_support:
            names = ", ".join(s.name for s in retail_support[:3])
            ratios = [f"{s.retail_foreign_ratio * 100:.0f}%" for s in retail_support[:3]]
            lines.append(f"  👥 개인 지지: {names} (흡수 {', '.join(ratios)})")
        if retail_fomo:
            names = ", ".join(s.name for s in retail_fomo[:3])
            lines.append(f"  🚨 개인 추격: {names} (위험!)")
        if retail_driven:
            names = ", ".join(s.name for s in retail_driven[:3])
            lines.append(f"  ⚠️ 개인 주도: {names}")
